...
; #AutoIt3Wrapper_Res_requestedExecutionLevel=requireAdministrator dòng này để có thể ConsoleWrite
...
#include <_HttpRequest.au3> 		; Thực hiện Request
; Check quyền Admin
If (Not IsAdmin()) Then
...
Global $API[6]
$API[0] = "/lol-champions/v1/owned-champions-minimal"       ; Lấy danh sách tướng hiện có
...
Func whereAmI() ; Check xem đã trong trận chưa
	...
	Switch $rq
		Case '"InProgress"'     ; Đang trong trận
			_HttpRequest_ConsoleWrite("Đang trong trận" & @CRLF)
			Stop()
			Return 1
		Case '"Matchmaking"'     ; Đang tìm trận
			_HttpRequest_ConsoleWrite("Đang tìm trận" & @CRLF)
			Sleep(2000)
			Return 2
		Case '"Lobby"'             ; Chưa tìm trận
			_HttpRequest_ConsoleWrite("Đang trong phòng chờ" & @CRLF)
			Sleep(2000)
			Return 3
		Case '"ChampSelect"'     ; Đang trong phần chọn tướng
			_HttpRequest_ConsoleWrite("Đang chọn tướng" & @CRLF)
		...
		Case '"None"'             ; Đang ở ngoài Menu chính
			_HttpRequest_ConsoleWrite("Đang ở ngoài menu chính" & @CRLF)
			Sleep(2000)
			Return 5
		Case '"ReadyCheck"'
			_HttpRequest_ConsoleWrite("Đang ở phần chấp thuận trận đấu" & @CRLF)
			$PICKED = False
			Return 6
		Case Else                 ; Không biết đang ở đâu luôn
			_HttpRequest_ConsoleWrite("Không biết đang ở đâu luôn: " & $rq & @CRLF)
			Sleep(2000)
			Return 7
	EndSwitch
EndFunc   ;==>whereAmI
...
Func Start()
	_HttpRequest_ConsoleWrite("+ Func Start" & @CRLF)
	...
	If $ACCEPT And $PICK_LOCK And $CHAMP_ID Then
		_HttpRequest_ConsoleWrite("Auto Accept + Auto Pick" & @CRLF)
		While 1
	...
	ElseIf $PICK_LOCK = False Then
		_HttpRequest_ConsoleWrite("Auto Accept" & @CRLF)
	...
	ElseIf $PICK_LOCK And $CHAMP_ID Then
		_HttpRequest_ConsoleWrite("Auto Pick" & @CRLF)
		While 1
			If $RUNNING = False Then
				ExitLoop
			EndIf
...
EndFunc   ;==>SelectSearchResult
Func _Exit()     ; Thoát khỏi TOOL
	$RUNNING = False
	Exit
EndFunc   ;==>_Exit
#EndRegion	Điều khiển GUI
